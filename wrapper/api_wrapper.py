...
class OlisAPI:
    ...
    def get_legislator_chief_sponsored_bills(
        ...
        endpoint = self._json_format(
            utils.get_endpoint(utils.Operation.GET_LEG_CHIEF_BILLS).format(session_key, legislator_key))
        return self.session.get(endpoint).json()
    ...
    def get_legislator_sponsored_bills_enrolled(
        ...
        endpoint = self._json_format(
            utils.get_endpoint(utils.Operation.GET_LEG_BILLS_ENROLLED).format(session_key, legislator_key))
        return self.session.get(endpoint).json()
